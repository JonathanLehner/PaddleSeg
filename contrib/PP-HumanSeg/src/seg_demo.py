...
def parse_args():
    ...
    parser.add_argument( # J: not used anywhere?
        '--vertical_screen',
...
def seg_video(args):
    ...
    mask_dir = args.save_dir.split(".")[0] # J: path without .avi
    if(not os.path.exists(mask_dir)): # J: folder might not exist, if not there then files cannot be saved
        os.mkdir(mask_dir) # J: seems this is too slow so we need to wait a little before writing files
        import time 
        time.sleep(1)
    ...
    with tqdm(total=total_frames) as pbar:
        ...
        while cap_img.isOpened():
            ...
            # J: E2FGVI needs masks
            mask_filename = f"{mask_dir}/{img_frame_idx:06d}.png"
            print(mask_filename)
            gray = cv2.cvtColor(out, cv2.COLOR_BGR2GRAY)
            blur = cv2.GaussianBlur(gray, (5, 5), 0)
            ret, thresh = cv2.threshold(blur,230,255,cv2.THRESH_BINARY_INV)
            kernel = np.ones((7, 7), np.uint8)
            dilated_img = cv2.dilate(thresh, kernel, iterations=10)
            cv2.imwrite(mask_filename, dilated_img)
